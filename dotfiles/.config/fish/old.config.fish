...
set -x PYTEST_ADDOPTS -x --ff --nf --pdb --pdbcls=IPython.terminal.debugger:TerminalPdb
...
# FIXME define functions or delete them:
# abbr -a git-mr-base git merge-base develop (git branch --show-current)
# abbr -a git-mr-files git diff (git merge-base develop (git branch --show-current))..HEAD
